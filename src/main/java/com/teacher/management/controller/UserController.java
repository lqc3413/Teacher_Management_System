...
public class UserController {
    ...
    public Result<?> login(@RequestBody Map<String, String> loginForm) {
        ...
        // 校验角色是否匹配（admin=1, teacher=2）
        if (role != null) {
            Long expectedRoleId = "admin".equals(role) ? 1L : 2L;
            if (!expectedRoleId.equals(user.getRoleId())) {
                String roleName = "admin".equals(role) ? "管理员" : "教师";
                return Result.error("该账号不是" + roleName + "角色，请切换登录方式");
            }
        }
